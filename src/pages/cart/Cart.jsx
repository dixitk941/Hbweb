        ...
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0">
          {cartItems.length === 0 ? (
            <div className="text-center py-10">
              <h2 className="text-lg font-semibold" style={{ color: mode === 'dark' ? 'white' : 'gray' }}>
                Your cart is empty. <br />
                <span className="text-xl">Add items to view your cart.</span>
              </h2>
            </div>
          ) : (
            <>
              <div className="rounded-lg md:w-2/3">
                {cartItems.map((item, index) => {
                  const { title, price, description, coverImageUrl } = item;
                  return (
                    <div key={index} className="justify-between mb-6 rounded-lg border drop-shadow-xl bg-white p-6 sm:flex sm:justify-start" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '' }}>
                      ...
                      <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                        <div className="mt-5 sm:mt-0">
                          ...
                          <h2 className="text-sm" style={{ color: mode === 'dark' ? 'white' : '' }}>{description}</h2>
