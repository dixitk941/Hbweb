...
function Dashboard() {
    ...
    const { mode, products, users, orders } = context;
    ...
    return (
        <Layout>
                        ...
                        {hasOrders && (
                            <div className="p-4 md:w-1/4 sm:w-1/2 w-full">
                                <div className="border-2 hover:shadow-purple-600 shadow-[inset_0_0_10px_rgba(0,0,0,0.6)] bg-gray-100 border-gray-300 px-4 py-3 rounded-xl" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '' }}>
                                    ...
                                    <h2 className="title-font font-medium text-3xl text-black fonts1" style={{ color: mode === 'dark' ? 'white' : '' }}>{orders.length}</h2>
            ...
            <DashboardTab products={products} users={users} orders={orders} mode={mode} />
        </Layout>
    );
}
